...
const INVESTIGATION_URL_PATTERN =
    /^[a-z0-9+.-]+:\/\/investigations\/([^/?#]+)/i;
...
export function notificationDataUrl(
...
export async function registerForPushNotificationsAsync(
    ...
    if (Platform.OS === "web") {
        return {
            status: "skipped",
            reason: "Push registration is only enabled for native devices.",
        };
    }
    if (!notificationsSupportedInCurrentShell()) {
        return {
            status: "skipped",
            reason: "Remote push is unavailable in Expo Go. Use a development build instead.",
        };
    }
    const Notifications = getNotificationsModule();
    if (!Notifications) {
        return {
            status: "skipped",
            reason: "expo-notifications is not available in this shell.",
        };
    }
    if (!Device.isDevice) {
        return {
            status: "skipped",
            reason: "Push registration requires a physical device.",
        };
    }
    ensureNotificationHandler();
    ...
    if (!projectId) {
        return {
            status: "skipped",
            reason: "Missing Expo project ID for push registration.",
        };
    }
    ...
    try {
        ...
        if (!response.ok) {
            return {
                status: "error",
                reason: "The backend rejected the push token.",
            };
        }
