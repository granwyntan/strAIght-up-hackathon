...
def _llm_quote_review(source: SourceAssessment, quote: str) -> QuoteVerificationOutput | None:
    return generate_structured_output(
        ...
        ),
        {
            ...
            "quote": quote,
            "source_text_excerpt": (source.extractedText or source.snippet)[:2500],
            "instructions": [
...
def verify_quotes(sources: list[SourceAssessment]) -> list[SourceAssessment]:
    verified_sources: list[SourceAssessment] = []
    for index, source in enumerate(sources):
        ...
        llm_review = None
        if candidate_quote and index < 10:
            llm_review = _llm_quote_review(source, candidate_quote)
